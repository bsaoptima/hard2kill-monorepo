#!/usr/bin/env bash
# Copy the built Vite client bundle (client-dist/) into the running cs16 container.
# Survives container restarts until the container is recreated.
# Prereq: run ./scripts/build-client.sh first to produce client-dist/.
set -euo pipefail
...
# Non-hashed top-level assets (favicon, logo) if present — copy anything else in dist root
find "$DIST" -maxdepth 1 -type f ! -name 'index.html' -print0 | while IFS= read -r -d '' f; do
  docker cp "$f" "$CONTAINER:/xashds/public/$(basename "$f")"
done

echo "[install-client] Done. HTTP server picks up new files on next request (no restart needed)."
